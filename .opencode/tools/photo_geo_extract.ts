...
function resolveRoot() {
  ...
  return fs.existsSync(path.join(relativeRoot, "exif_vision.py")) ? relativeRoot : FALLBACK_ROOT
}
...
export default tool({
  description: "Extract GPS, OCR, and visual clues from an image path or URL.",
  args: {
  ...
  async execute(args) {
    ...
    const script = path.join(root, "exif_vision.py")
