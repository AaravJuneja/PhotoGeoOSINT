...
function resolveRoot() {
  ...
  return fs.existsSync(path.join(relativeRoot, "osint_artifact_extract.py")) ? relativeRoot : FALLBACK_ROOT
}
...
export default tool({
  ...
  args: {
    input: tool.schema.string().describe("Linux path, Windows path, or HTTP/HTTPS URL to a challenge artifact"),
  },
  async execute(args) {
    const root = resolveRoot()
    const script = path.join(root, "osint_artifact_extract.py")
